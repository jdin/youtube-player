...
})})}firstUpdated(props){this.height=window.document.body.clientHeight-40;window.addEventListener("resize",()=>{this.height=window.document.body.clientHeight-40})}}customElements.define("my-video",MyVideo);var myVideo={MyVideo:MyVideo};const idsPromise=fetch("https://www.googleapis.com/youtube/v3/playlistItems?part=id&maxResults=50&playlistId=PLn0kOTV0__XZlNhrnUsVzNzA6XAgSm2rh&key=AIzaSyDNad_BkgZoFz4PKwCLxEXwKM-fLA92dnk").then(resp=>resp.json()).then(v=>v.items).then(array=>array.map(video=>video.id.videoId)),videos=idsPromise.then(ids=>ids.map(id=>html$1`<my-video class="box" src=${id}></my-video>`)),wakeLock=async()=>{if("getWakeLock"in navigator){try{// Create a wake lock for the type we want.
