...
events:{onReady:()=>{console.log("here");console.log(player);player.playVideo()}}})}var player$1={play:play};class MyVideo extends LitElement{constructor(){super();this.height=320;this.src=null;this.isPlaying=!1}static get properties(){return{height:Number,src:String,isPlaying:Boolean}}get width(){return 16*this.height/9}render(){return html$1`
        ...
        `}onClick(){this.isPlaying=!0;setTimeout(()=>{let frame=this.shadowRoot.getElementById("frame");play(frame,this.src,this.height,this.width,()=>{console.log("set isplaying to false for "+this.src);this.isPlaying=!1;// this.shadowRoot.removeChild(frame);
})})}firstUpdated(props){this.height=window.document.body.clientHeight-40;window.addEventListener("resize",()=>{this.height=window.document.body.clientHeight-40})}}customElements.define("my-video",MyVideo);var myVideo={MyVideo:MyVideo};const idsPromise=fetch("https://www.googleapis.com/youtube/v3/search?part=id"+"&channelId=UChEqdYH7NbCgE9aRJuACL3w&maxResults=50&type=video"+"&publishedAfter=2019-04-03T00%3A00%3A00Z"+"&key=AIzaSyDNad_BkgZoFz4PKwCLxEXwKM-fLA92dnk").then(resp=>resp.json()).then(v=>v.items).then(array=>array.map(video=>video.id.videoId)),videos=idsPromise.then(ids=>ids.map(id=>html$1`<my-video class="box" src=${id}></my-video>`)),wakeLock=async()=>{if("getWakeLock"in navigator){try{// Create a wake lock for the type we want.
const wakeLockObj=await navigator.getWakeLock("screen");console.log("\uD83D\uDC4D","getWakeLock",wakeLockObj);wakeLockObj.createRequest()}catch(ex){console.error("\uD83D\uDC4E","getWakeLock",err)}}else{console.log("no wakelock")}};//.then(console.log);
wakeLock();class MyApp extends LitElement{render(){return html$1`
    <style>
      .container {
        display: flex;
        align-items: center;
