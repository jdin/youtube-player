...
if(!resolveDoc){resolveDoc=document.implementation.createHTMLDocument("temp");resolveDoc.base=resolveDoc.createElement("base");resolveDoc.head.appendChild(resolveDoc.base);resolveDoc.anchor=resolveDoc.createElement("a");resolveDoc.body.appendChild(resolveDoc.anchor)}resolveDoc.base.href=baseURI;resolveDoc.anchor.href=url;return resolveDoc.anchor.href||url}/**
   ...
   */function pathFromUrl(url){return url.substring(0,url.lastIndexOf("/")+1)}var resolveUrl$1={resolveUrl:resolveUrl,resolveCss:resolveCss,pathFromUrl:pathFromUrl};const useShadow=!window.ShadyDOM,useNativeCSSProperties=!!(!window.ShadyCSS||window.ShadyCSS.nativeCss),useNativeCustomElements=!window.customElements.polyfillWrapFlushCallback;/**
    ...
    */let syncInitialRender=!1;/**
                                       ...
                                       * rendering globally.
                                       * @return {void}
                                       */const setSyncInitialRender=function(useSyncInitialRender){syncInitialRender=useSyncInitialRender};var settings={useShadow:useShadow,useNativeCSSProperties:useNativeCSSProperties,useNativeCustomElements:useNativeCustomElements,get rootPath(){return rootPath},setRootPath:setRootPath,get sanitizeDOMValue(){return sanitizeDOMValue},setSanitizeDOMValue:setSanitizeDOMValue,get passiveTouchGestures(){return passiveTouchGestures},setPassiveTouchGestures:setPassiveTouchGestures,get strictTemplatePolicy(){return strictTemplatePolicy},setStrictTemplatePolicy:setStrictTemplatePolicy,get allowTemplateFromDomModule(){return allowTemplateFromDomModule},setAllowTemplateFromDomModule:setAllowTemplateFromDomModule,get legacyOptimizations(){return legacyOptimizations},setLegacyOptimizations:setLegacyOptimizations,get syncInitialRender(){return syncInitialRender},setSyncInitialRender:setSyncInitialRender};let dedupeId=0;/**
                   * @constructor
                   * @extends {Function}
                   * @private
...
for(let i=0,e;i<e$.length;i++){e=e$[i];if(e.parentNode){e.parentNode.removeChild(e)}cssText+=e.textContent}return cssText}/**
   ...
   */ /* eslint-disable valid-jsdoc */ /**
                                        ...
                                        */const wrap$1=window.ShadyDOM&&window.ShadyDOM.noPatch&&window.ShadyDOM.wrap?window.ShadyDOM.wrap:n=>n;var wrap$2={wrap:wrap$1};function isPath(path){return 0<=path.indexOf(".")}/**
...
// in v1 Shadow DOM.
forwardReadOnlyProperties(DomApiNative.prototype,["parentNode","firstChild","lastChild","nextSibling","previousSibling","firstElementChild","lastElementChild","nextElementSibling","previousElementSibling","childNodes","children","classList"]);forwardProperties(DomApiNative.prototype,["textContent","innerHTML"])}const DomApi=DomApiImpl,dom=function(obj){obj=obj||document;if(obj instanceof DomApiImpl){return(/** @type {!DomApi} */obj)}if(obj instanceof EventApi){return(/** @type {!EventApi} */obj)}let helper=obj.__domApi;if(!helper){if(obj instanceof Event){helper=new EventApi(obj)}else{helper=new DomApiImpl(/** @type {Node} */obj)}obj.__domApi=helper}return helper};/**
                                   ...
                                   */var polymer_dom={matchesSelector:matchesSelector,EventApi:EventApi,DomApi:DomApi,dom:dom,flush:flush$1,addDebouncer:enqueueDebouncer};const bundledImportMeta$1={...import.meta,url:new URL("../../node_modules/%40polymer/polymer/lib/legacy/legacy-element-mixin.js",import.meta.url).href};let styleInterface=window.ShadyCSS;/**
...
customStyleInterface$1.processStyles()},/**
   ...
   */_shouldPropertyChange(property,value,old){return mutablePropertyChange$1(this,property,value,old,this.mutableData)}};/**
    ...
    *   arrays
    */var mutableDataBehavior={MutableDataBehavior:MutableDataBehavior,OptionalMutableDataBehavior:OptionalMutableDataBehavior};const Base=LegacyElementMixin(HTMLElement).prototype;var polymerLegacy={Base:Base,Polymer:Polymer,html:html},KEY_IDENTIFIER={"U+0008":"backspace","U+0009":"tab","U+001B":"esc","U+0020":"space","U+007F":"del"},KEY_CODE={8:"backspace",9:"tab",13:"enter",27:"esc",33:"pageup",34:"pagedown",35:"end",36:"home",32:"space",37:"left",38:"up",39:"right",40:"down",46:"del",106:"*"},MODIFIER_KEYS={shift:"shiftKey",ctrl:"ctrlKey",alt:"altKey",meta:"metaKey"},KEY_CHAR=/[a-z0-9*]/,IDENT_CHAR=/U\+/,ARROW_KEY=/^arrow/,SPACE_KEY=/^space(bar)?/,ESC_KEY=/^escape$/;/**
                           * Transforms the key.
...
if(event.defaultPrevented){return}}}},_triggerKeyHandler:function(keyCombo,handlerName,keyboardEvent){var detail=Object.create(keyCombo);detail.keyboardEvent=keyboardEvent;var event=new CustomEvent(keyCombo.event,{detail:detail,cancelable:!0});this[handlerName].call(this,event);if(event.defaultPrevented){keyboardEvent.preventDefault()}}};var ironA11yKeysBehavior={IronA11yKeysBehavior:IronA11yKeysBehavior},Utility={distance:function(x1,y1,x2,y2){var xDelta=x1-x2,yDelta=y1-y2;return Math.sqrt(xDelta*xDelta+yDelta*yDelta)},now:window.performance&&window.performance.now?window.performance.now.bind(window.performance):Date.now};/**
    * @param {HTMLElement} element
    * @constructor
...
if(state.lastRenderedIndex!==void 0&&"function"===typeof value.then&&value===previousValues[i]){continue}// We have a Promise that we haven't seen before, so priorities may have
// changed. Forget what we rendered before.
state.lastRenderedIndex=void 0;Promise.resolve(value).then(resolvedValue=>{const index=state.values.indexOf(value);// If state.values doesn't contain the value, we've re-rendered without
// the value, so don't render it. Then, only render if the value is
// higher-priority than what's already been rendered.
if(-1<index&&(state.lastRenderedIndex===void 0||index<state.lastRenderedIndex)){state.lastRenderedIndex=index;part.setValue(resolvedValue);part.commit()}})}});/**
                               ...
                               */var until$1={until:until};let player=null,prevCallback;const setFullscreen=doc=>{const requestFullScreen=doc.requestFullScreen||doc.mozRequestFullScreen||doc.webkitRequestFullScreen;if(requestFullScreen){requestFullScreen.call(doc)}},cancelFullscreen=doc=>{const cancelFullScreen=doc.exitFullscreen||doc.mozCancelFullScreen||doc.webkitExitFullscreen||doc.msExitFullscreen;if(cancelFullScreen){cancelFullScreen.call(doc)}};function play(frame,videoId,height,width,stopRequestCallback){if(player){player.stopVideo();prevCallback();// player.stopVideo();
}prevCallback=stopRequestCallback;player=new YT.Player(frame,{height:height,width:width,videoId:videoId,events:{onReady:e=>{player=e.target;player.playVideo();setFullscreen(e.target.a)},onStateChange:event=>{if(event.data===YT.PlayerState.PLAYING){setFullscreen(event.target.a)}else if(2===event.data){cancelFullscreen(window.document)}}}})}var player$1={play:play};class MyVideo extends LitElement{constructor(){super();this.height=320;this.src=null;this.isPlaying=!1}static get properties(){return{height:Number,src:String,isPlaying:Boolean}}static get styles(){return css`
          :host { display:block; }
        ...
        `}onClick(){this.isPlaying=!0;setTimeout(()=>{let frame=this.shadowRoot.getElementById("frame");play(frame,this.src,this.height,this.width,()=>{this.isPlaying=!1;// this.shadowRoot.removeChild(frame);
})})}firstUpdated(props){this.height=window.document.body.clientHeight-40;window.addEventListener("resize",()=>{this.height=window.document.body.clientHeight-40})}}customElements.define("my-video",MyVideo);var myVideo={MyVideo:MyVideo};const idsPromise=fetch("https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50"+"&playlistId=PLn0kOTV0__XZlNhrnUsVzNzA6XAgSm2rh&key=AIzaSyDNad_BkgZoFz4PKwCLxEXwKM-fLA92dnk").then(resp=>resp.json()).then(v=>v.items).then(array=>array.map(video=>video.snippet.resourceId.videoId)),videos=idsPromise.then(ids=>ids.map(id=>html$1`<my-video class="box" src=${id}></my-video>`));//.then(console.log);
class MyApp extends LitElement{static get styles(){return css`
        :host { display:block; }
        .container {
          display: flex;
          align-items: center;
